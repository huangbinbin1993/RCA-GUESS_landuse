...
void soilparameters(Soiltype& soiltype,int soilcode,double soildepth) {
	...
	*/
	...
	if (soildepth >= 600.0) {
		...
		soiltype.wsats[1] = min((soildepth-SOILDEPTH_UPPER),SOILDEPTH_LOWER) * data[soilcode-1][6];
	} else {
		...
		soiltype.wsats[0] = SOILDEPTH_UPPER * data[soilcode-1][6]/2.0;
		soiltype.wsats[1] = min((soildepth-SOILDEPTH_UPPER/2.0),SOILDEPTH_LOWER) * data[soilcode-1][6];
	}
