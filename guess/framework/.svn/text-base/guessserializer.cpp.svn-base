...
// New function to detect file exist (mcw120216)
// Using stat() should be the most efficient way compared with
// access() or other io stream methods
...
GuessSerializer::GuessSerializer(const char* directory, int my_rank, int num_processes) {
	try {
		...
		// Now we detect meta.bin first before creating it (mcw120216)
		// (instead of using the hard-wired rank number, as the sub-domain simulated by the 
		// first rank is not necessary land, in this case GUESS is not called)
		if (!file_exists(meta_file_path(directory))) {
