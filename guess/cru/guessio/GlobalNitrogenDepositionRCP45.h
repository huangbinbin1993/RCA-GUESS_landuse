...
//
// Functionality to retrieve data from the archive is provided by class GlobalNitrogenDepositionRCP45Archive.
// The following public functions are provided:
//
// bool open(char* filename)
//   Attempts to open the specified file as a fast data archive. The format must be
//   exactly compatible with this version of GlobalNitrogenDepositionRCP45.h (normally the archive and
...
//   errors. open() with no argument is equivalent to open("GlobalNitrogenDepositionRCP45.bin").
//
// void close()
...
//
// bool getnext(GlobalNitrogenDepositionRCP45& obj)
...
//
// bool getindex(GlobalNitrogenDepositionRCP45& obj)
...
//   GlobalNitrogenDepositionRCP45 data;
//   bool success,flag;
//
//   // Retrieve all records in sequence and print values of longitude and latitude:
//
//   success=ark.open("GlobalNitrogenDepositionRCP45.bin");
//   if (success) {
//      flag=ark.rewind();
//      while (flag) {
...
struct GlobalNitrogenDepositionRCP45 {
...
const long GLOBALNITROGENDEPOSITIONRCP45_NRECORD=59191;
const int GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH=1914;
const int GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH=7;
const int GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE=670;
unsigned char GLOBALNITROGENDEPOSITIONRCP45_HEADER[GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE-4]={
...
class GlobalNitrogenDepositionRCP45Archive {
...
private:
	...
	long recno;
	long datano;
	unsigned char pindex[GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH];
	unsigned char pdata[GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH];
	bool iseof;
	...
	void getindex(long n) {
		...
		fseek(pfile,GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH*(n-recno),SEEK_CUR);
		fread(pindex,GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH,1,pfile);
		datano=pindex[GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH-4]*0x1000000+pindex[GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH-3]*0x10000+
			pindex[GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH-2]*0x100+pindex[GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH-1];
		recno=n+1;
		iseof=(recno==GLOBALNITROGENDEPOSITIONRCP45_NRECORD);
	}

	void getdata() {

		fseek(pfile,GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH*-recno+(datano-GLOBALNITROGENDEPOSITIONRCP45_NRECORD)*GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,SEEK_CUR);
		fread(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,1,pfile);
		fseek(pfile,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH*(GLOBALNITROGENDEPOSITIONRCP45_NRECORD-datano-1)+GLOBALNITROGENDEPOSITIONRCP45_INDEX_LENGTH*recno,SEEK_CUR);
	}
	...
	bool initialise(const char* filename) {
		...
		pheader=new unsigned char[GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE-4];
		if (!pheader) {
		...
		::rewind(pfile);
		fread(pheader,GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE-4,1,pfile);
		for (i=0;i<GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE-4;i++) {
			if (pheader[i]!=GLOBALNITROGENDEPOSITIONRCP45_HEADER[i]) {
				printf("Format of %s incompatible with this version of GlobalNitrogenDepositionRCP45.h\n",filename);
				fclose(pfile);
				pfile=NULL;
				delete pheader;
				return false;
		...
		::rewind(pfile);
		fseek(pfile,GLOBALNITROGENDEPOSITIONRCP45_HEADERSIZE+GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH*GLOBALNITROGENDEPOSITIONRCP45_NRECORD,SEEK_CUR);
		recno=0;
		iseof=false;
...
public:
	...
	GlobalNitrogenDepositionRCP45Archive() {
		pfile=NULL;
	}
	...
	bool open() {
		return open("GlobalNitrogenDepositionRCP45.bin");
	}
	...
	bool getnext(GlobalNitrogenDepositionRCP45& obj) {
		...
		for (i=131;i>=0;i--) obj.NOyWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,29,1e-005,0);
		for (i=131;i>=0;i--) obj.NOyDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,29,1e-005,0);
		for (i=131;i>=0;i--) obj.NHxWet[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,29,1e-005,0);
		for (i=131;i>=0;i--) obj.NHxDry[i]=popreal(pdata,GLOBALNITROGENDEPOSITIONRCP45_DATA_LENGTH,29,1e-005,0);
	...
	bool getindex(GlobalNitrogenDepositionRCP45& obj) {
		...
		if (!GLOBALNITROGENDEPOSITIONRCP45_NRECORD || !pfile) return false;
		...
		bool not_done=true;
		long direction=1;
		long offset=GLOBALNITROGENDEPOSITIONRCP45_NRECORD/2+GLOBALNITROGENDEPOSITIONRCP45_NRECORD%2;
		long thisrecord=0-GLOBALNITROGENDEPOSITIONRCP45_NRECORD%2;

		while (not_done) {
			thisrecord+=offset*direction;
			if (thisrecord>=GLOBALNITROGENDEPOSITIONRCP45_NRECORD) thisrecord=GLOBALNITROGENDEPOSITIONRCP45_NRECORD-1;
