...
void ndemand(Patch& patch, Vegetation& vegetation) {
	...
	while (vegetation.isobj) {
		...
		if (indiv.pft.lifeform == TREE) {
			indiv.sapndemand = max(0.0, indiv.cmass_sap / (cton_leaf_opt * indiv.pft.cton_sap_avr / indiv.pft.cton_leaf_avr) - indiv.nmass_sap) * ((1.0 + (double)date.day)/date.ndayyear);
		}
...
void aet_water_stress(Patch& patch, Vegetation& vegetation, const Day& day) {
	...
	for (int p=0; p<npft; p++) {
		...
		if (!date.diurnal()) {
			...
			ppft.gcbase_gross = ppft.wstress ? max(gc_monteith(ppft.wsupply, patch.eet_net_veg), 0.0) : 0;
		}
		else if (day.isend) {
			...
			ppft.gcbase_gross = ppft.wstress_day ? max(gc_monteith(ppft.wsupply,	patch.eet_net_veg), 0.0) : 0;
		}
	}
...
void water_scalar(Patch& patch, Vegetation& vegetation, const Day& day) {
	...
	for (int p=0; p<npft; p++) {
		...
		if (day.isend) {
			...
			// Convert from sum to mean on last day of year
			if (date.islastday && date.islastmonth) {
				ppft.wscal_mean /= (double)date.ndayyear;
			}
		}
	}
...
void npp(Patch& patch, Climate& climate, Vegetation& vegetation, const Day& day) {
	...
	// Ben RCA-GUESS 070614: added computation of patch-level canopy conductance
	...
	while (vegetation.isobj) {
		...
		*/
		...
		// Increment cumulative sum of daily LAI for stand
		stand.pft[indiv.pft.id].dlaiphen[date.day]+=indiv.lai*indiv.phen/(double)stand.nobj;

		vegetation.nextobj();
	}
	
	// Convert patch canopy conductance sum to average on last day of month
	// and store in stand monthly gc array

	if (date.islastday) {
		patch.gc/=(double)date.ndaymonth[date.month];
		stand.gc_out[date.month]+=patch.gc/(double)stand.nobj;
	}
}
