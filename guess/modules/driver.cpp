 ...
 *
 *  Differs from standard version due to leap years.
 */
void interp_monthly_means(double mvals[12], double dvals[366]) {

	Date d; // Date object used for interpolation (local to this function) // leap year is not considered yet, mcw
	double nday, dayct;
	int thismonth, lastmonth;

	d.init(1);

	nday = (double)(d.middaymonth[0] - (d.middaymonth[11] - date.ndayyear));
	thismonth = 0;
	lastmonth = 11;
	dayct = (double)(date.ndayyear - d.middaymonth[11]);

	// Perform interpolation

	while (d.year == 0) {
		if (d.day == d.middaymonth[d.month]) {
			if (d.month == 11) // December
				nday = (double)(d.middaymonth[0] + date.ndayyear - d.middaymonth[11]);
			else
				nday = (double)(d.middaymonth[d.nextmonth()] -
					d.middaymonth[d.month]);
			thismonth = d.nextmonth();
			lastmonth = d.month;
			dayct = 0.0;
		}
		dvals[d.day] = (mvals[thismonth] - mvals[lastmonth]) / nday * dayct +
			mvals[lastmonth];

		d.next_day();

		if (d.day>370) fail("d.day=%d\n",d.day); //BLARP!!!
		dayct++;
	}
}
...
void interp_monthly_totals(double mvals[12], double dvals[366]) {
	...
	// RCA: we use the global date object because of leap years
	//Date date;
...
void distribute_ndep(const double* mndry, const double* mnwet,
	...
	// RCA: we use the global date object to get the correct number of days per month
	// (due to leap days)
	//	Date date;
	int start_of_month = 0;
...
//  (From Dieter Gerten 021121)
	...
	for (m=0; m<12; m++) {
		...
		// BLARP!! Make sure at least one rainday in month
...
void dailyaccounting_climate(Climate& climate) {
	...
	// For output of forcing data (RCA-GUESS)
	
	if (date.dayofmonth==0) {
		climate.mtemp_out[date.month]=climate.temp/(double)date.ndaymonth[date.month];
		climate.mco2_out[date.month]=climate.co2/(double)date.ndaymonth[date.month];
		climate.mprec_out[date.month]=climate.prec;
	}
	else {
		climate.mtemp_out[date.month]+=climate.temp/(double)date.ndaymonth[date.month];
		climate.mco2_out[date.month]+=climate.co2/(double)date.ndaymonth[date.month];
	...
	if (climate.temp < 5.0 && climate.chilldays <= date.ndayyear)
		climate.chilldays++;
...
///		dailyaccounting_gridcell_crop(gridcell,pftlist);
	...
	// Hot fix for the invalid value of climate.temp: mw120216
	if (climate.temp>-100.0 && climate.temp<100.0)
		climate.dtemp_31.add(climate.temp);
...
void dailyaccounting_stand(Stand& stand) {
	...
	// Initialise canopy conductance and daily LAI cumulative sums (for RCA-GUESS)
	if (date.day==0) {		
		for (int m = 0; m < 12; m++) {
			stand.gc_out[m] = 0.0;
		}
...
void dailyaccounting_patch(Patch& patch) {
	...
	soil.mwcontupper+=soil.wcont[0];
	soil.mwcontlower+=soil.wcont[1];
	
//	if (date.calendar_year == 1980 && date.month == 1)
		//fprintf(stdout, "I am here");

	// On last day of month, calculate mean content of upper soil layer
	if (date.islastday) {

		soil.mwcontupper/=(double)date.ndaymonth[date.month];
		soil.mwcontlower/=(double)date.ndaymonth[date.month];
	...
	if (date.islastday) {
		...
		soil.mtemp_out[date.month]=soil.mtemp;
	}
}
...
void daylengthinsoleet(Climate& climate) {
	...
	if (!climate.doneday[date.day]) {
		...
		climate.qo[date.day] = QOO * (1.0 + 2.0 * 0.01675 *
							cos(2.0 * PI * ((double)date.day + 0.5) / (double)date.ndayyear)); // Eqn 2
		double delta = -23.4 * DEGTORAD * cos(2.0 * PI * ((double)date.day + 10.5) / (double)date.ndayyear);
	...
	if (climate.instype == SUNSHINE) {		// insolation is percentage sunshine
		...
		else if (climate.instype == SWRAD || climate.instype == SWRAD_TS) {
			// include correction for albedo
			climate.rad = climate.insol * (1.0 - BETA) * averaging_period;
		}
		else if (climate.instype==PAR) {
			climate.rad = climate.insol/FRADPAR;
		}
	...
	// For output during spinup (RCA-GUESS)
	if (date.dayofmonth==0) {
		climate.mpar_out[date.month]=climate.par/(double)date.ndaymonth[date.month];
	}
	else {
		climate.mpar_out[date.month]+=climate.par/(double)date.ndaymonth[date.month];
	}
